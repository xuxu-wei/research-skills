...
| `0.10.0` | GitHub Marketplace 安装成功并识别 50 个技能、21 个 reviewer，且只启用 Git 通道；但 `openai_plugin_dev.py verify --channel github` 报告“no Git marketplace cache matches the expected source identity”。 | Windows 开发工作树检出的文本多为 CRLF，GitHub Marketplace 缓存为 LF；当前校验器对完整文件做逐字节比较，因换行风格不同拒绝了语义一致的安装包。版本、Registry、清单数量及 `git diff --no-index` 的实际文件差异均未显示插件内容漂移。 | 以后可在副本比较中仅对已识别的文本文件规范化 CRLF/LF 后再比较，同时继续严格检查二进制内容与完整文件清单；当前不修改校验器、不复现、不增加专项测试，改用 Git 安装后的 fresh-agent 只读发现烟测验证实际加载。 | `record_only_owner_prioritization_pending` |
