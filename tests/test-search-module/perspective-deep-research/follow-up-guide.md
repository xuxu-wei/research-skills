# Perspective Deep Research 测试

请在当前 `research-skills` 项目中启动 `$perspective-orchestrator`，完成以下测试。

- 只读输入：`tests/test-perspective/init-perspective.md`
- 证据检索模式：`deep_research`
- 唯一输出目录：`tests/test-search-module/perspective-deep-research/runs/run-002/`
- 会话显示名：`codex-test-perspective-deep-research-r2`
...
使用给定输入启动完整 Perspective 工作流。不得修改输入文件、插件源码或其他
测试目录。证据阶段强制使用 Deep Research，不得以普通 Web Search、普通对话
总结或多次 focused synthesis 代替实际 Deep Research 返回报告。
...
3. 在输出根目录写入 `run-status.yaml`，记录 case、实际插件版本、工作流、输入、
   输出、检索模式、当前阶段、两份接续文件路径和下一步；然后暂停，等待真实
   Deep Research 报告，不得自行模拟报告。
4. 在对话中明确返回接续文件和 `run-status.yaml` 的准确路径。

收到并保存真实 `deep-research-report-vNNN.md` 后，在同一工作流会话中继续：
核验问题覆盖、主张—来源可追溯性、完整链接、相近工作、相反证据、适用范围和
未解决问题；保留原始报告；把通过核验的内容整合为新的证据产物。原始请求、
guide、报告和搜索历史不得进入最终 evaluator 输入。
