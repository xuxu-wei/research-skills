...
全部 8 个 Skill 均允许 Codex 按任务自动匹配。可以直接描述需求，例如“将这些笔记写成研究报告”“改善这份草稿的可读性”或“检查研究设计是否能回答问题”，无需先写出技能名称。实际匹配效果由所有者在新任务中验证；也可用技能名称明确指定。

`sap-writer` 的正文与参考资料使用中文，按研究问题串联共同定义、分析方法、缺失与敏感性、统计推断及结果呈现，并按需安排附录。框架可随研究设计和用户格式裁剪，生成的 SAP 语言遵循任务要求。
...
统一评估的正文与参考资料使用中文，发现描述保持英文。自动匹配与显式调用都保留独立评估要求；它按任务加载共同价值、科学依据、文章类型，以及标书、分析计划或编辑决策标准，评论来信和更正线索采用专门核查。
...
上一次入口精简删除了独立的研究改进规划与方法学预检入口，由已有 `research-evaluator` 承接，收敛为 8 个 Skill。该轮保留统一评估技能及其参考资料，没有新增流程或迁入重复提示。

当前版本保持 8 个 Skill，进一步开放统一评估与 SAP 的自动匹配，并将发现描述中的核心用途前置。SAP 写作流程与参考资料改用中文，吸收研究问题、共同定义、分析模块和附录相互衔接的组织方式；统一评估的正文、评分标准和独立性规则保持不变。历史版本中保留显式调用的记录仍描述当时的设计。
