...
确定性验证是每次改动的默认门槛；LLM 测试是风险触发项，不是每次发布的全量固定门槛。测试前先按下表记录变更类别、受影响工作流、触发理由和拟运行命令。不得因为版本号变化、发布动作或历史上曾测试过某工作流，就自动调用 LLM 重跑所有完整工作流。

| 类别 | 典型改动 | 确定性门槛 | Fresh discovery | Fresh-agent forward test |
|---|---|---|---|---|
| A. 文档/报告 | README、ROADMAP、开发文档、无行为影响的报告 | 相关文档合同与审计 | 不需要 | 不需要 |
| B. 确定性实现 | 生成器、审计器、测试 harness、行为等价重构、生成 Registry/报告 | 受影响单元/合同测试、仓库审计、转换器、插件验证器 | 仅发现面改变时需要 | 不需要 |
| C. 单工作流行为 | 某一工作流的 Skill 指令、模型读取模板、状态/路由、Rubric/Gate、Reviewer 输入边界 | B 类门槛 + 该工作流专项合同 | 仅入口名称、描述、调用策略或通道改变时需要 | 只运行受影响工作流的最小代表场景 |
| D. 共享行为 | 跨工作流编辑/评估合同、共享 Reviewer、公共状态或隔离策略 | 全部受影响消费者的确定性合同 | 发现面改变时需要 | 按受影响消费者矩阵运行；不得默认扩大到无关工作流 |
| E. 通道发布 | 已通过 Local 验收的同一源码身份切换到 GitHub | 通道 `verify`、发布合同 | GitHub 新任务需要 | 同一源码身份不重复行为测试 |

### LLM 测试触发条件

以下改动至少属于 C 类：改变模型执行的 Role、Procedure、Required Inputs、Output Contract、停止条件、委派 brief、模型读取模板、Reviewer Rubric/Gate、Orchestrator 边/状态，或改变 writer/reviewer 隔离边界。只改自然语言说明但会改变模型决策时，也按行为改动处理。

以下改动本身不触发 LLM：纯文档、格式、注释、确定性测试、生成的 Registry/报告、未改变模型可见合同的 harness 修复，以及已有确定性等价证明覆盖的重构。若同一提交同时包含行为改动，仍按最高风险类别执行。

全量多工作流 LLM 测试只在以下情况运行：所有者明确要求；共享合同实际影响所有完整工作流；或 Major/里程碑发布明确把全量运行列为验收目标。普通 Minor/Patch 或单工作流版本更新不得仅因“要发布”而升级为全量 LLM 测试。

执行顺序保持由低成本到高成本：

1. **局部静态验证**：受影响 Skill 的 `quick_validate.py`、只读 lint 或 schema 检查。
2. **插件级验证**：相关合同测试、仓库审计、转换器和插件验证器。
3. **Fresh discovery smoke（条件触发）**：仅当发现面或安装通道改变时，在重装后的新任务中确认目标 Skill、插件基本版本和入口可发现。
4. **Fresh-agent forward test（条件触发）**：仅对行为受影响的工作流，使用原始 fixture 和新的输出目录运行最小代表场景，不向测试任务泄漏预期缺陷或修复答案。
...
Local 已对某一精确源码身份完成的行为测试可以用于同一源码身份的 GitHub 发布判断；切换通道后只补 GitHub discovery smoke，不重复调用 LLM。任何后续模型可见合同变更都会使受影响工作流的行为证据失效；无关工作流不因此自动失效或重跑，但交付记录必须明确标注“未受影响、未重跑”，不得把历史结果表述为当前版本的全量运行。
...
只有当前风险分层要求的 Local 验收通过后才能发布；A/B 类不虚构 LLM 验收要求，C/D 类只要求受影响工作流的定向行为验收：
...
6. GitHub 通道验证后新建任务运行 discovery smoke。只有该任务确认正式安装版本和入口正确，才能宣布发布完成；若发布的是已经通过 Local 定向行为验收的同一源码身份，不重复运行 fresh-agent 行为测试。
...
- `verify` 绑定正确通道和源码版本。
- 已记录变更类别、受影响工作流、LLM 触发判断和实际命令；没有把单工作流改动扩大成无关工作流的全量 LLM 测试。
- 若风险分层要求行为测试，其结果来自重装后的 fresh task，fixture 未被修改；A/B 类明确记录“不需要”，不伪造运行结果。
- 同一源码身份从 Local 切到 GitHub 时只补 discovery smoke，不重复行为测试。
