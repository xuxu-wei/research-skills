...
description: "Independently triage the complete supplied material inventory for article readiness and routing without drafting."
---
# article-readiness-triage

## Purpose

Before manuscript writing, decide whether the complete supplied material inventory supports responsible drafting. Do not normalize context, retrieve literature, audit methods, or draft content.
...
- Read every file declared in the frozen material inventory that can establish the research question, methods actually used, executed results, displays, or reporting context. Do not infer absence from a minimal summary when a supplied technical report, result table, figure, or analysis output can answer the readiness question.
- Honor a declared sole semantic authority when meanings or version labels conflict, while retaining compatible execution and result assets. Report unresolved semantic conflict instead of silently discarding those assets.
- Article drafting does not inherently require raw participant-level data or rerunning the analysis. A traceable executed technical report plus interpretable result outputs may establish writing readiness; unavailable raw data or code is a reproducibility/verification limitation unless the intended claim cannot be supported without reanalysis.
...
- `minimal_intake_summary`: study_topic, apparent_study_type, complete material inventory, semantic-authority declaration, stated target journal, and obvious missing items.
- Every declared material that could change readiness, including supplied technical reports, result tables, figures, and executed analysis outputs.
...
| primary_results | All article types (may be supplied through a traceable technical report, result tables/figures, or executed statistical output) |
...
Match study design, evidence strength, and material volume to the output enum: full multi-analysis study → `original_article`; one bounded finding → `brief_report`; preliminary/exploratory work → `research_letter`; method/tool → `methods_article`; dataset/resource → `data_descriptor`; case/series → `case_report`; no original data → `review`.
...
Return `realistic`, `ambitious_but_possible`, or `mismatch`, with the observed basis rather than prestige assumptions.
...
Do not equate limitations with unreadiness, inflate a bounded finding into an original article, pass absent primary results, lower the bar to proceed, or perform context-builder normalization.

## Verification

Confirm all seven minimum input categories, a required action per blocking gap, study-matched article type, evidence-based journal realism, and a specific route.
