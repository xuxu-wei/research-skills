...
- Run this skill only in a fresh independent subagent or delegated thread. Never assess text in the same agent context that drafted, revised, or polished it.
- Receive frozen artifact IDs, file paths, and versions. Treat all source artifacts as read-only and write only the Language Assessment Report.
- Do not edit, rewrite, polish, or fix the assessed text. Return locatable findings and revision priorities to the orchestrator or calling user.
- Do not use parent-thread hidden reasoning, expected conclusions, prior reviewer outputs, or prior scores. In re-assessment mode, receive only the latest frozen text plus an anonymized issue list and revision delta when issue-resolution checking is required.
- Report the exact files read, sections assessed, scope limitations, and the independent reviewer instance identifier.
- If a fresh subagent cannot be created, return `independent_review_pending` with a self-contained continuation brief and stop. Never fall back to inline self-assessment.
...
Every Language Assessment Report must include this provenance block in addition to the language-specific findings:
...
Execute as a fresh independent subagent when called by:
...
The subagent must receive the frozen manuscript text, target discipline, target journal (if any), and scope. Do not rely on parent session context. For re-assessment, do not expose the prior overall score or decision.
