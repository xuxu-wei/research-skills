...
- Write only an evaluation report. Do not draft, rewrite, polish, fix, or otherwise modify any evaluated source.
...
- anonymized prior must-fix issue list，如为 re-evaluation（不得包含 prior score、overall rationale 或 decision）
...
- 用户目标和约束；
- evidence summary，如有；
- anonymized prior must-fix issue list 与 revision delta，如为再评价；不得读取 prior score、overall rationale 或 decision。
...
- novelty claim 不成立；
- 用户目标明显不匹配；
- data-access、method 或 implementation blocker；
- proposal 结构缺失导致无法评审；
...
以 fresh、隔离子 agent 或 delegated thread 形式显式派发。
