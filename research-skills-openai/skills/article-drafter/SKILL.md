...
`05_audit/**`, and the assigned `09_revisions/**` plan. Write only versioned
...
1. Validate the blueprint, context, grounding, methods audit, identity anchor,
   display assets, supplement index, and optional revision plan.
...
4. Draft Introduction as known -> gap -> objective using grounded citations.
5. Draft Discussion around the contribution and primary answer: findings,
   literature context, limits, implications/generalizability, and conclusion.
...
7. In revision mode, output a complete next Markdown version and leave the
   controller to maintain the separate response and delta.
...
order, display/supplement consistency, versioned lineage, no reviewer prose, and
DOCX parity/render routing when applicable.
