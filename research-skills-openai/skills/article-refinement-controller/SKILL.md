...
1. Explicitly delegate `academic-language-assessor` (standalone-preflight mode) to a fresh independent subagent with the frozen draft → get detailed issues
...
5. Re-assess with a new fresh `academic-language-assessor` subagent that cannot see the prior score or decision
6. If still failing → mark `language_status: needs_professional_editing`

If either language assessor cannot run in a fresh independent subagent or delegated thread, return `independent_review_pending` with a continuation brief and stop the language route. Never assess language inline.

### Step 8: Return for Fresh Re-Evaluation

Return the new manuscript version, revision delta, and an anonymized must-fix list to the orchestrator. The orchestrator must delegate `article-evaluator` to a new independent evaluator instance. Do not call the evaluator from this controller, do not reuse the prior evaluator context, and do not expose prior scores or decisions. The orchestrator compares sealed evaluation reports only after the new report is complete.
...
- Do not compare evaluation rounds or decide `stop_no_gain`; that is orchestrator-owned after fresh re-evaluation.
...
- Updated manuscript returns to the orchestrator for a fresh evaluator instance with no prior scores or decisions
