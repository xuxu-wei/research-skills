...
- Explicitly delegate `academic-language-assessor` to a fresh independent subagent for English, Chinese, or bilingual language quality evaluation before final package assembly and after any language polishing pass.
- Evaluation and review must be isolated: explicitly assign readiness triage, proposal evaluation, SAP evaluation, every re-evaluation, and every review-panel role to fresh independent subagents or delegated threads.
...
Explicitly assign `proposal-readiness-triage` to a fresh independent subagent or delegated thread. Do not execute the gate inline.
...
- `needs_methodology_preflight`: explicitly delegate `methodology-statistics-preflight` to a fresh independent subagent if it blocks drafting or SAP.
...
Explicitly assign `proposal-evaluator` to a fresh independent subagent or delegated thread. Do not evaluate inline.
...
- re-evaluation by a new `proposal-evaluator` instance in a fresh independent subagent or delegated thread. It receives the latest and previous frozen versions, an anonymized must-fix list, and the revision delta, but no prior score, rationale, or decision.
...
1. Explicitly delegate `methodology-statistics-preflight` to a fresh independent subagent.
2. If preflight blocks SAP, set `sap_status: preflight_blocked` and stop SAP branch.
3. Call `sap-writer`.
4. Explicitly assign `sap-evaluator` to a fresh independent subagent or delegated thread.
...
Create one fresh independent subagent or delegated thread per reviewer role and start the complete panel concurrently. Default panel tier is `standard_panel` with 5 reviewers. `lightweight_panel` has 3 reviewers and must include a domain expert, methodology/statistics reviewer, and submission-guard reviewer. `full_panel` has 7 reviewers. For medicine, clinical practice, or public health, use `practicing-clinician reviewer` as the domain expert role. Wait for every selected reviewer to finish before aggregation; preserve dissent and do not expose any review to another reviewer.
...
Before applying the label route, inspect the sealed reports and panel summary for any credible `FATAL` or blocking finding. Such a finding overrides a supportive label: do not proceed to final assembly as ready/minor. Route to Step 7 for a fixable major repair, `not_ready` when required artifacts or work are unavailable, or `reject_or_redesign` when unfixable.
...
Before assembly, explicitly delegate a fresh `academic-language-assessor` instance against the frozen final proposal. If polishing changes the proposal, create a new version and repeat the required fresh proposal evaluation before continuing.
...
- prepare a self-contained ChatGPT Deep Research continuation package and pause;
