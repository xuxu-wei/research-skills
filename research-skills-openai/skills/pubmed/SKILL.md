---
name: pubmed
description: "Search PubMed for biomedical papers by keyword, author, journal, MeSH term, DOI, or PMID. Use ChatGPT/Codex built-in Search by default for targeted source-grounded lookup; use bundled NCBI E-utilities scripts only as an optional reproducible local Codex fallback."
---
# PubMed Search

Search and retrieve biomedical literature while keeping PubMed source scope explicit.

## Default ChatGPT/Codex Route

1. Translate the question into precise English terms, MeSH terms, field qualifiers, authors, journals, DOI, or PMID as applicable.
2. Use built-in Search with PubMed/NCBI as the required source scope. Open the PubMed record or linked full text before citing it.
3. Verify title, authors, journal, publication date, PMID, DOI, abstract availability, and whether the record actually matches the requested scope.
4. For current or newly indexed literature, require live search. If only indexed/cached search is available, report the freshness limitation.
5. Return direct PubMed links and stable identifiers. Distinguish metadata/abstract review from full-text review.

Use the bundled E-utilities scripts only when running in a local Codex environment and the task needs reproducible batch retrieval or machine-readable output. Script failure must not cause invented results; fall back to built-in Search or report the access limitation.

## Optional Local Codex Quick Reference
