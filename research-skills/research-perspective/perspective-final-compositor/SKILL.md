...
metadata:
  hermes:
    tags: [research-perspective, final, compositor, compliance, audit]
    related_skills:
...
May Call: medical-journal-review（如可用；仅用于额外合规审计，不得覆盖本 skill 的 return route）
...
### submission-readiness-report.md
```
Ready for submission: yes / no / conditional / outlet-targeting-only
Remaining risks: [列表]
...
- 越权做实质修改
- 编辑掩盖实质问题
- 把 Generic Profile 场景误标为 submission-ready；只能声明 ready for outlet targeting
