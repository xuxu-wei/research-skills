...
description: Control targeted proposal revision loops after independent proposal evaluation.
  Converts evaluator feedback into revision plans, coordinates proposal-drafter updates,
  enforces file-centered version lineage, requests independent re-evaluation, compares
  revision deltas, and decides whether to continue, accept, reject, or stop for no
  gain.
version: 1.2.0
author: Xuxu Wei
license: MIT
...
修订后必须调用 `proposal-evaluator` 进行 re-evaluation，并派发给新的隔离、独立 evaluator 子 agent。
...
以下任务必须通过 `delegate_task` 派发给隔离子 agent：

- proposal re-evaluation（派发 `proposal-evaluator`）——子 agent 不得访问前轮 evaluator 的评分；
