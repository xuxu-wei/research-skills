...
description: Draft and maintain a proposal file from a proposal-ready context brief.
  Creates the initial proposal document, updates it in response to evaluator feedback,
  preserves version lineage, and keeps claims aligned with available evidence and
  user constraints. In revision mode, produces a separate reviewer response file;
  proposal body must be kept clean of response-to-reviewer language.
  Does not evaluate its own proposal.
version: 1.3.0
author: Xuxu Wei
license: MIT
metadata:
...
- `proposal-orchestrator` 的 `references/proposal-writing-methodology.md`：基于 7 份已获批范例的 proposal 撰写方法论。通过 `skill_view(name="proposal-orchestrator", file_path="references/proposal-writing-methodology.md")` 加载。
- `references/anti-pattern-checklist.md`：proposal 完稿后的 anti-pattern 自检清单。handoff 前逐条检查。通过 `skill_view(name="proposal-drafter", file_path="references/anti-pattern-checklist.md")` 加载。
