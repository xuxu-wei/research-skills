...
- user constraints
- previous SAP evaluation report, if re-evaluation
...
- previous evaluation and SAP delta report, if re-evaluation.
...
For SAP revision re-evaluation, compare previous and current SAP versions.
...
This skill should be called by `proposal-orchestrator` or `sap-refinement-controller` through `delegate_task` as an isolated subagent.
