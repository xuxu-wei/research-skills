...
description: Evaluate whether an existing research idea is ready to enter proposal
  drafting. Produces a readiness decision, identifies blocking gaps, and routes the
  workflow to drafting, clarification, idea refinement, methodology preflight, or
  stop. This skill is an evaluator gate and must not draft the proposal.
version: 1.1.0
author: Xuxu Wei
license: MIT
metadata:
...
- known ethical, regulatory, privacy, or feasibility blockers.
...
本 skill 本身应由 `proposal-orchestrator` 以 `delegate_task` 隔离子 agent 形式调用。
