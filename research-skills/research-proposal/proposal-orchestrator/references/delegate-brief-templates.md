...
Evaluator and reviewer tasks must be run through `delegate_task` as isolated subagents.
...
- Previous evaluation report, if re-evaluation: {{previous_evaluation_report_path_or_text}}
...
- Previous SAP evaluation report, if re-evaluation: {{previous_sap_evaluation_report_path_or_text}}
...
Dispatch all selected reviewers at once through `delegate_task(tasks=[...])`.
...
- Dispatch readiness, proposal evaluation, SAP evaluation, and SAP re-evaluation as single isolated `delegate_task` calls.
- Dispatch review panel reviewers through one `delegate_task(tasks=[...])` call.
