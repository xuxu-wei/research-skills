...
metadata:
  hermes:
    tags: [research-proposal, SAP, revision, re-evaluation, statistics, methodology]
    related_skills:
...
Use `delegate_task` to assign a fresh isolated `sap-evaluator` subagent.

The re-evaluation brief must include the updated SAP file, previous SAP file or version, previous SAP evaluation report, SAP delta report, preflight report, endpoint/metric definitions, data description, and user constraints.
