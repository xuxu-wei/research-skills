...
metadata:
  hermes:
    tags: [research-article, article, orchestration, manuscript, writing, evaluation, revision, peer-review, submission]
    related_skills:
...
Use this skill when the user has existing research data, results, and study design background information and wants to produce a submission-ready manuscript package.
...
- Use `methodology-statistics-preflight` for quick feasibility screening before `article-methods-statistics-auditor`.
- Use `academic-language-assessor` for English, Chinese, or bilingual language quality evaluation during evaluation and refinement.
...
- Evaluation will be isolated: use `delegate_task` for readiness triage, methods-statistics-auditor, claim-auditor, evaluator, and every review-panel reviewer.
...
This skill owns the contract references for the research-article package. Other article skills load these orchestrator-owned contract references through the runtime's available reference-loading adapter:

```text
skill_view(name="article-orchestrator", file_path="references/artifact-contracts.md")
skill_view(name="article-orchestrator", file_path="references/artifact-naming-and-directory-rules.md")
skill_view(name="article-orchestrator", file_path="references/handoff-validation.md")
```

If `skill_view(...)` is not available in the current runtime, load the same files directly from `article-orchestrator/references/` or pass their contents in the delegated brief. Do not introduce a `_shared` skill.
...
Use `delegate_task` to assign an isolated `article-readiness-triage` subagent.
...
Use `delegate_task` to assign an isolated `article-methods-statistics-auditor` subagent.
...
Use `delegate_task` to assign an isolated `article-claim-auditor` subagent.
...
Use `delegate_task` to assign an isolated `article-evaluator` subagent.

Evaluator calls `academic-language-assessor` for the Language & Academic Register dimension, and performs Supplementary Audit.
...
Decision labels: `accept`, `revise`, `reject`, `stop_no_gain`.
...
- `reject` → stop
- `stop_no_gain` → stop (re-evaluation only)
...
Language polishing: call `academic-language-assessor` → save `08_evaluations/language-assessment-vNNN.md` → drafter fixes → save `09_revisions/round-NNN/language-change-log-rNNN.md` → re-assessment. If a changed draft is saved, create a new manuscript version with `change_type: language_only`.
...
Use `delegate_task(tasks=[...])` to dispatch all reviewers concurrently in isolated subagents.
...
For biomedical manuscripts, `article-cover-letter` delegates a cover-letter-only review to `medical-journal-review` and records the apparent article tier. The delegate receives only the cover letter.
...
Call `article-submission-compositor`.
