# Opportunity Map

## Summary

- Number of opportunities:
- Overall evidence status:
- Main limitations:

## Opportunities

| ID | Type | Title | Description | Evidence confidence | Novelty risk | Guideline alignment | Why it matters | Feasibility concerns | Recommended generation paths |
|---|---|---|---|---|---|---|---|---|---|

## Downstream Notes

### For multi-path-idea-generator

- 

### For methodology-statistics-preflight

- 

### For isolated idea-evaluator

- 

### For idea-portfolio-assembler

- 

### For proposal-context-brief-builder / proposal-orchestrator

- 

### For proposal-evaluator

- 
