...
### DeepSearch Prompt Adjustment (Path B)

| Level | 调整 |
|-------|------|
| Standard | 使用 `templates/deepsearch-prompt-template.md` 默认版本 |
| Divergent | Phase 1 拓宽检索范围、增加相邻领域来源、降低时效性限制 |
| Focused | 缩减至单阶段 targeted retrieval、提高时效性要求、增加验证性要求 |

详细规则见 `references/deepsearch-prompt-rules.md` 的 Level-specific adjustments 章节。
