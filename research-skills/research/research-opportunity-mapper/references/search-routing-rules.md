...
### Existing Wiki / Knowledge Base

`llm-wiki` is optional. Use only when the user provides or maintains a relevant wiki.
...
- `expanded_retrieval`: use only when the topic is broad, evidence conflicts, novelty/gap claims are central, or the task is clinical/high-stakes.
